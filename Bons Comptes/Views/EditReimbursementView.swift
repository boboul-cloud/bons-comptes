//
//  EditReimbursementView.swift
//  Bons Comptes
//

import SwiftUI

struct EditReimbursementView: View {
    ...
    init(campaign: Campaign, reimbursement: Reimbursement) {
        self.campaign = campaign
        self.reimbursement = reimbursement
        _amount = State(initialValue: String(format: "%.2f", reimbursement.amount))
        _date = State(initialValue: reimbursement.date)
        _fromID = State(initialValue: reimbursement.fromID)
        _toID = State(initialValue: reimbursement.toID)
        _paymentMethodID = State(initialValue: reimbursement.paymentMethodID)
        _notes = State(initialValue: reimbursement.notes)
        _isPartial = State(initialValue: reimbursement.isPartial)
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(NSLocalizedString("edit_reimbursement", comment: ""))
    ...
    func saveReimbursement() {
        ...
        updated.notes = notes
        updated.isPartial = isPartial

        store.updateReimbursement(updated)
        dismiss()
    }
}
